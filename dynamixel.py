...
class Dynamixel:
    ...
    def _enable_torque(self, motor_id, tries = 10):
        dxl_comm_result = None
        if dxl_comm_result != COMM_SUCCESS:
            cnt = 0
            while cnt < tries:
                dxl_comm_result, _ = self.packetHandler.write1ByteTxRx(self.portHandler, motor_id,
    ...
    def _disable_torque(self, motor_id, tries=10):
        dxl_comm_result = None
        if dxl_comm_result != COMM_SUCCESS:
            cnt = 0
            while cnt < tries:
    ...
    def _process_response(self, dxl_comm_result: int, dxl_error: int, motor_id: int, tries=5):
        if dxl_comm_result != COMM_SUCCESS:
            raise ConnectionError(f"dxl_comm_result for motor {motor_id}: {self.packetHandler.getTxRxResult(dxl_comm_result)}")
        elif dxl_error == 128:
            # voltage error
    ...
    def set_operating_mode(self, motor_id: int, operating_mode: OperatingMode, tries=10):
        dxl_comm_result = None
        if dxl_comm_result != COMM_SUCCESS:
            cnt = 0
            while cnt < tries:
                dxl_comm_result, _ = self.packetHandler.write2ByteTxRx(self.portHandler, motor_id,
    ...
    def set_pwm_limit(self, motor_id: int, limit: int, tries=10):
        dxl_comm_result = None
        if dxl_comm_result != COMM_SUCCESS:
            cnt = 0
            while cnt < tries:
    ...
    def set_velocity_limit(self, motor_id: int, velocity_limit, tries=10):
        dxl_comm_result = None
        if dxl_comm_result != COMM_SUCCESS:
            cnt = 0
            while cnt < tries:
                dxl_comm_result, _ = self.packetHandler.write4ByteTxRx(self.portHandler, motor_id,
